...
def apply_filters(
    ...
    if review_count and "review_count" not in ignore:
        if review_count == "Below 5":
            filtered = filtered[filtered["Reviews Count"] < 5]
        else:
            filtered = filtered[filtered["Reviews Count"] >= int(review_count)]
    if review_rating and "review_rating" not in ignore:
        filtered = filtered[filtered["Total Score"] >= float(review_rating)]
                    ...
                    children=[
                        ...
                        html.Div(
                            ...
                            children=[
                                ...
                                dcc.Dropdown(id="review-rating-filter", placeholder="⭐ Review rating", clearable=True),
                                dcc.Dropdown(
...
@app.callback(
    ...
    Output("review-rating-filter", "options"),
...
def update_table(search, industry, category, suburb, accessibility, review_count, review_rating, fav_only, _trig, _auth):
    ...
    selected = {
        ...
        "review_count": review_count, "review_rating": review_rating,
