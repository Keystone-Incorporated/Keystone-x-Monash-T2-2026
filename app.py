                    ...
                    children=[
                        ...
                        html.Div(
                            ...
                            children=[
                                ...
                                multi_filter("category", "📂 Category", options=CATEGORY_OPTIONS),
...
def msf_set_active(_toggle_clicks, _outside_clicks, current_active):
    ...
    if trig == "msf-outside-trigger":
        # A click landed outside every filter — close whichever panel is open.
    ...
    if isinstance(trig, dict) and trig.get("type") == "msf-toggle":
        idx = trig["index"]
        # Clicking the already-open filter's toggle closes it; any other opens it
        # (and implicitly closes whatever else was open, since only one is tracked).
...
def msf_sync(all_options, search, _all_clicks, _none_clicks, _clear_clicks, current):
    ...
    if trig_type == "msf-all":
        # Select everything currently visible (respects the search box)
        ...
        # Options were recomputed by cross-filtering: drop stale selections
...
def update_table(search, industry, category, suburb, accessibility, review_count, _clear_clicks, fav_only, _trig, _auth):
    # A "✕ clear" click goes straight to this callback rather than waiting for
    # its own checklist's "value" prop to round-trip first — that extra hop was
    # what made the *other* filters' option lists look stale until a second
    # interaction. Zero out the cleared filter's selection immediately here.
    trig = ctx.triggered_id
    ...
    # Cross-filtering: every filter's available options are computed from the
    # dataset filtered by every *other* currently-selected filter, so each
    # dropdown only ever offers options that still have matching results.
    ...
    # Give each row a stable "id" (its original dataframe index) so Dash's
    # active_cell reports a reliable "row_id" — active_cell's "row" is only the
    # index within the currently *displayed* page/sort order, which breaks once
    # you page forward or sort a column. "row_id" stays correct regardless.
    for rec in records:
        rec["id"] = rec["_row_idx"]
...
def update_modal(active_cell, _fav, _com, close_clicks, current_business):
    ...
    # "row_id" is the stable id we set on each record (its original dataframe
    # index) — unlike active_cell["row"], it stays correct across pagination
    # and client-side sorting, since it doesn't depend on display position.
